...
#
# Copyright 2016-2022 Michael Buesch <m@bues.ch>
#
...
class _XmlFactoryBuilder:
	...
	def pushFactory(self, xmlFactory):
		parentFactory = self.__factoryList[-1] if self.__factoryList else None
		self.__factoryList.append(xmlFactory)
		xmlFactory.builder = self
		xmlFactory.parser_open(
			tag=((self.__tags[-1] if self.__tags else None),
...
class XmlFactory(ABC):
	...
	class Tag:
		...
		FLAG_ATTR_LINE_BREAK	= 0x1
		FLAG_EMIT_EMPTY_ATTRS	= 0x2
		FLAG_EMIT_EMPTY_TAG	= 0x4
		FLAG_USE_CDATA		= 0x8
	...
	def parser_beginTag(self, tag):
		print("[XML-parser - %s] Unhandled tag: <%s>" % (
	...
	def parser_data(self, data):
		data = data.strip()
		if data:
			print("[XML-parser - %s] Unhandled data: %s" % (
	...
	def compose(self,
		...
		try:
			...
			raise XmlFactory.Error("Failed to compose XML data: %s" % str(e))
	...
	def parse(self, xmlText):
		try:
			...
			parser.feed(xmlText)
			parser.close()
		except (ElementTree.ParseError, UnicodeError) as e:
			raise XmlFactory.Error("Failed to parse XML data: %s" % str(e))
		return True
